...
- **執行檔大小退步（已二分，處理中）**：hello world 在 `74fa648`（9/13）是 **48,840 位元組**，
  今天是 **508,304 位元組**，`.text` 從 12,693 漲到 315,433。二分結果第一個跳升在
  `52913a0`「feat(lib): java.util.function」（74,384 → 105,328），之後每加一個程式庫就再
  跳一階。產生的 C 仍是 67,285 行（prelude 一向如此），但反射**成員**表並沒有被寫進去
  （`tyfield`／`tymethod`／`tyannotation` 皆為 0，閘門有效），所以問題在於 prelude 不再被
  LTO 丟掉，而不是中繼資料被寫出。**這是未解決的退步，不是完成的優化**；「執行檔很小」目前
  的正確說法見文件站那張表。
